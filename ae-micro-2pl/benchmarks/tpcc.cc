...
static mcs_rwlock dis_range_lock[64][10];
static int32_t last_no_o_ids[64][10];

static inline void 
acquire_dis_range_lock(int wid, int dis, mcs_rwlock::qnode_t* node)
{
  ALWAYS_ASSERT(wid > 0 && wid <= 64);
  ALWAYS_ASSERT(dis > 0 && dis <= 10);

  dis_range_lock[wid - 1][dis - 1].start_write(node);
}

static inline void 
release_dis_range_lock(int wid, int dis, mcs_rwlock::qnode_t* node)
{
  ALWAYS_ASSERT(wid > 0 && wid <= 64);
  ALWAYS_ASSERT(dis > 0 && dis <= 10);

  dis_range_lock[wid - 1][dis - 1].end_write(node);
}
...
tpcc_worker::txn_new_order()
{
  ...
  std::vector <uint> itemIDs, supplierWarehouseIDs, orderQuantities, itemPrices;
  // uint itemIDs[15], supplierWarehouseIDs[15], orderQuantities[15];
  bool allLocal = true;
  for (uint i = 0; i < numItems; i++) {
    // itemIDs[i] = GetItemId(r);
    itemIDs.push_back(GetItemId(r));

    if (likely(g_disable_xpartition_txn ||
               NumWarehouses() == 1 ||
               RandomNumber(r, 1, 100) > g_new_order_remote_item_pct)) {
      // supplierWarehouseIDs[i] = warehouse_id;
      supplierWarehouseIDs.push_back(warehouse_id);
    } else {

      uint s_wid = warehouse_id;
      while(s_wid == warehouse_id) {
        s_wid = RandomNumber(r, 1, NumWarehouses());  
      }

      supplierWarehouseIDs.push_back(s_wid);
      allLocal = false;
    }
    // orderQuantities[i] = RandomNumber(r, 1, 10);
    orderQuantities.push_back(RandomNumber(r, 1, 10));
  }

  std::sort(itemIDs.begin(), itemIDs.end());
  std::sort(supplierWarehouseIDs.begin(), supplierWarehouseIDs.end());
  ...
  try {
    ...
    ALWAYS_ASSERT(tbl_warehouse(warehouse_id)->get_for_read(txn, Encode(obj_key0, k_w), obj_v));
    ...
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      ...
      ALWAYS_ASSERT(tbl_item(1)->get_for_read(txn, Encode(obj_key0, k_i), obj_v));
      item::value v_i_temp;
    ...
    ALWAYS_ASSERT(tbl_customer(warehouse_id)->get_for_read(txn, Encode(obj_key0, k_c), obj_v));
...
tpcc_worker::txn_delivery()
{
  ...
  try {
    ...
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      acquire_dis_range_lock(warehouse_id, d, &qnode[d - 1]);
    }

    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      
      // acquire_dis_range_lock(warehouse_id, d, &qnode[d - 1]);
      const new_order::key k_no_0(warehouse_id, d, last_no_o_ids[warehouse_id - 1][d - 1]);
      ...
      if (unlikely(!k_no))
        continue;
      last_no_o_ids[warehouse_id - 1][d - 1] = k_no->no_o_id + 1; // XXX: update last seen
    ...
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      release_dis_range_lock(warehouse_id, d, &qnode[d - 1]);
    }
