...
class INFO {
public:
    static int txn_read_abort, txn_write_abort, txn_absent_abort;
};
...
int CONFLICT_WINDOW = 3;
int F_CONFLICT_WINDOW = 100;
std::vector<int64_t> conflict_list;
std::vector<int64_t> conflict_list_flight;

string COUNTRY_FILE = "./seats-data/table.country.csv";
string AIRPORT_FILE= "./seats-data/table.airport.csv";
string AIRLINE_FILE="./seats-data/table.airline.csv";
string FLIGHTS_PER_AIRPORT_FILE="./seats-data/histogram.flights_per_airport";
static unsigned g_txn_workload_mix[] = {25,0,0,25,25,25};//{10,10,35,20,10,15};//{20, 5, 5, 20, 45, 5}; //{15,5,5,15,55,5};//{10,4,4,10,68,4};//{20, 5, 5, 20, 45, 5};
...
static inline unsigned int NumPartitions() {
    return (unsigned int)scale_factor;
}
...
static void loadCountry(){
    ...
    for(uint32_t i = 0 ; i < NumPartitions(); i++){
        ...
        flight_list.push_back(new std::vector<flight::value*>());
...
#define DEFN_TBL_ACCESSOR_X(name) \
    ...
    protected: \
        ...
        { \
            INVARIANT(wid >= 1 && wid <= NumPartitions()); \
            INVARIANT(tbl_ ## name ## _vec.size() == NumPartitions()); \
            ...
            partition_id(partition_id)
    {
        ...
        virtual void
            on_run_setup() OVERRIDE
            {
                ...
                for (int i = 0; i<CONFLICT_WINDOW;i++){
                    reservation_entry *inserted = (*all_inserted[partition_id-1])[i];
                    conflict_list.push_back(inserted->customer_id);
                }
                for (int i = 0; i<F_CONFLICT_WINDOW;i++){
                    reservation_entry *inserted = (*all_inserted[partition_id-1])[i];
                ...
                try {
                    for (uint i=1; i<=airport_list.size(); i++) {
                        ...
                        for(uint j = 1; j <= airport_distances[i-1]->size(); j++){
                            ...
                            ALWAYS_ASSERT(tbl_airport_distance(1)->get(txn, Encode(dk), obj_buf));
                        }
...
class country_callback : public abstract_ordered_index::scan_callback{
    public:
        ...
        std::vector<country::key>k_cos;
        std::vector<country::value>v_cos;
                ...
                for(int p_id = st_part; p_id <= ed_part; p_id++){
                    ...
                    for(int i = 0; i < TOTAL_DAY; i++){
                        ...
                        for(int fno = 0; fno < flights; fno++){
                            ...
                            flight::value *v = new flight::value();
...
class frequent_flyer_callback : public abstract_ordered_index::scan_callback{
    public:
        ...
        vector<frequent_flyer::key> k_ff;
        vector<frequent_flyer::value> v_ff;
...
class customer_callback :public abstract_ordered_index::scan_callback{
    public:
                ...
                const string &value)
        {
            ...
            UNUSED const seats_customer::key *k = Decode(keyp, temp);
            //printf("scan %ld\n" ,k->c_id);
            return true;
        }
};
...
seats_worker::txn_delete_reservation()
{
    ...
    try {
        ...
        // Raw piece: R seats_customer_name_idx
        if (rand <= PROB_DELETE_WITH_CUSTOMER_ID_STR) {
            ...
            if(tbl_seats_customer_name_idx(partition_id)->get_for_read(txn, Encode(obj_key0, k_c), obj_v)) {
        ...
        // Raw piece: R frequent_flyer_name_idx
        ...
        customerIndex++;
        if (customerIndex>=CONFLICT_WINDOW)
            customerIndex=0;
        int64_t fake_c_id = conflict_list[customerIndex];
        c_id = fake_c_id;

        flightIndex++;
        if (flightIndex>=F_CONFLICT_WINDOW)
            flightIndex=0;
        int64_t fake_f_id = conflict_list_flight[flightIndex];
        f_id = fake_f_id;

        // Conflict piece 1: D reservation/reservation_seat_idx
        ...
        tbl_flight(partition_id)->put(txn, Encode(str(), fake_k_f), Encode(str(), v_f_new));

        // Conflict piece 3: W seats_customer
        ...
        if(!found){
            printf("partition_id %u cid_id %lx\n", partition_id, c_id);
            ALWAYS_ASSERT(false);
        }
        ...
        tbl_seats_customer(partition_id)->put(txn, Encode(str(), fake_k_c), Encode(str(), v_c_new));

        // Conflict piece 4: W frequent_flyer
        if (al_id != 0) {
            ...
            v_ff_new.ff_iattr10--;
            tbl_frequent_flyer(partition_id)->put(txn, Encode(str(), k_ff), Encode(str(), v_ff_new));
        }
...
seats_worker::txn_find_flights()
{
    ...
    void *txn = db->new_txn(txn_flags | transaction_base::TXN_FLAG_READ_ONLY, arena, txn_buf());
    scoped_str_arena s_arena(arena);
    bool txn_ret = false;
    try {
...
seats_worker::txn_new_reservation()
{
    ...
    try {
        ...
        int64_t f_id = fake_f_id;//reserv->flight_id;
        ...
        // Conflict piece 1: I reservation/reservation_seat_idx
        ...
        tbl_reservation(partition_id)->insert(txn, Encode(str(), key_r), Encode(str(), val_r));
        ...
        /*const seats_customer::key k_c(partition_id, c_id);
          bool found = tbl_seats_customer(partition_id)->get(txn, Encode(obj_key0, k_c), obj_v);
        ...
        /*seats_customer::value v_c_new(*v_c);
          ...
          v_c_new.c_iattr10++;
          v_c_new.c_iattr11++;
          tbl_seats_customer(partition_id)->put(txn, Encode(str(), k_c), Encode(str(), v_c_new));*/
        ...
        flight::value v_f_temp;
        const flight::value *v_f = Decode(obj_v, v_f_temp);
        int64_t al_id = v_f->f_al_id;
        int64_t seats_left = v_f->f_seats_left;
        FORUNUSED(seats_left);
        ...
        ALWAYS_ASSERT(tbl_airline(partition_id)->get_for_read(txn, Encode(obj_key0, k_a), obj_v));
        airline::value v_a_temp;
        const airline::value *v_a = Decode(obj_v, v_a_temp);
        FORUNUSED(v_a);
...
seats_worker::txn_update_customer()
{
    ...
    // printf("Txn Update Customer\n");
    //uint64_t st = rdtsc();
    //return txn_result(true, 0);
    scoped_str_arena s_arena(arena);
    /*ALWAYS_ASSERT(partition_list->size() > 0);
      int index = RandomNumber(r, 1, partition_list->size());
      seats_worker* pick = (*partition_list)[index-1];
      reservation_entry* temp = pick->delete_queue.front();*/
    int64_t c_id = 0;
    if(RandomNumber(r, 1,100) < 90){
    ...
    try {
        ...
        if (RandomNumber(r, 1, 100) <= PROB_UPDATE_WITH_CUSTOMER_ID_STR) {
            ...
            UNUSED bool found = tbl_seats_customer_name_idx(partition_id)->get_for_read(txn, Encode(obj_key0, k_c), obj_v);
        ...
        k_c.c_id = c_id;
#if 0
        if (airport <3)printf("----------------------------------airport %ld\n", airport+1);
        seats_customer::key k_c_1(partition_id, (airport +2)<<48);
        customer_callback c;
        tbl_seats_customer(partition_id)->scan(txn, Encode(obj_key0, k_c), &Encode(obj_key1, k_c_1), c, s_arena.get());
#endif
#if 1
        customerIndex++;
        if (customerIndex>=CONFLICT_WINDOW)
        ...
        // Conflict piece 1: W seats_customer
        int64_t fake_c_id = conflict_list[customerIndex];
        seats_customer::key fake_k_c;
        fake_k_c.c_partition = partition_id;
        fake_k_c.c_id = fake_c_id;
        ...
        seats_customer::value v_c_temp;
        const seats_customer::value *v_c = Decode(obj_v, v_c_temp);
        uint64_t airport_id = v_c->c_base_ap_id;
        seats_customer::value v_c_new(*v_c);
        v_c_new.c_iattr00 = attr0;
        v_c_new.c_iattr01 = attr1;
        tbl_seats_customer(partition_id)->put(txn, Encode(str(), fake_k_c), Encode(str(), v_c_new));

        // Raw piece: R airline
        const airport::key k_a(airport_id);
        ALWAYS_ASSERT(tbl_airport(1)->get_for_read(txn, Encode(obj_key0, k_a), obj_v));
        airport::value v_a_temp;
        const airport::value *v_a = Decode(obj_v, v_a_temp);
        uint64_t co_id = v_a->ap_co_id;

        // Raw piece: R country
        const country::key k_co(co_id);
        ALWAYS_ASSERT(tbl_country(1)->get_for_read(txn, Encode(obj_key0, k_co), obj_v));
        country::value v_co_temp;
        UNUSED const country::value *v_co = Decode(obj_v, v_co_temp);

        // Conflict piece 2: W frequent_flyer
        if (RandomNumber(r, 1,100) <= PROB_UPDATE_FREQUENT_FLYER){
            ...
            for (uint i=0; i<callback.k_ff.size(); i++) {
                callback.v_ff[i].ff_iattr00 = attr0;
                callback.v_ff[i].ff_iattr01 = attr1;
                tbl_frequent_flyer(partition_id)->put(txn, Encode(str(), (callback.k_ff[i])), Encode(str(), callback.v_ff[i]));
            }
        }
#endif
        txn_ret = db->commit_txn(txn);
