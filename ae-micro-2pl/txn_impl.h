...
transaction<Protocol, Traits>::commit(bool doThrow)
{
  ...
  if (!is_snapshot()) {
    ...
    //       //WZG: unleash the read lock
    //       dbtuple *t = const_cast<dbtuple *>(it->get_tuple());
    //       // t->unlock();
    //       // if(it->get_lock_node()) {
    //       //   t->end_read(it->get_lock_node());
    //       //   delete it->get_lock_node();
    //       // }
    //       // printf("commit read unlock %lx\n", (uint64_t)t);
      ...
      for (; it != it_end; ++it) {
        ...
        if (it->is_insert()) {
          ...
          if (unlikely(ret.head_ != tuple)) {
            ...
            // ret.head_->lock(true);
            if (!ret.head_->start_write(get_tid()))
            {
    ...
    dbtuple::tuple_writer_t writer)
{
  ...
  // mcs_rwlock::qnode_t* node = new mcs_rwlock::qnode_t();
  typename lock_set_map::iterator it_r = find_write_lock_set(tuple);
  if (it_r == write_lock_set.end())
  {
    if (!tuple->start_write(get_tid(), not_sorted()))
    {
      const transaction_base::abort_reason r = transaction_base::ABORT_REASON_LOCK_CONFLICT;
      abort_impl(r);
      throw transaction_abort_exception(r);
...
transaction<Protocol, Traits>::do_tuple_read(
    const dbtuple *tuple, ValueReader &value_reader, bool readonly)
{
  INVARIANT(tuple);
  ...
  dbtuple::ReadStatus stat;
  {
    ...
    //WZG: should distiguish between "lock for write" and "lock for read"
    if(is_snapshot_txn) {
      ...
      if(readonly) {
        ...
        // mcs_rwlock::qnode_t* node = read_set.back().get_lock_node();
        //WZG: read lock
        typename lock_set_map::iterator it_r = find_read_lock_set(t_ptr);
        if (it_r == read_lock_set.end())
        {
          if (!t_ptr->start_read(get_tid(), not_sorted()))
          {
            const transaction_base::abort_reason r = transaction_base::ABORT_REASON_LOCK_CONFLICT;
            abort_impl(r);
            throw transaction_abort_exception(r);
        ...
        // printf("do_tuple_read acquire read lock %lx\n", (uint64_t)t_ptr);
        ...
        // mcs_rwlock::qnode_t* node = read_for_write_set.back().get_lock_node();
        //WZG: write lock
        typename lock_set_map::iterator it_r = find_write_lock_set(t_ptr);
        if (it_r == write_lock_set.end())
        {
          if (!t_ptr->start_write(get_tid(), not_sorted()))
          {
            const transaction_base::abort_reason r = transaction_base::ABORT_REASON_LOCK_CONFLICT;
            abort_impl(r);
            throw transaction_abort_exception(r);
...
template <typename ValueReader>
bool
transaction<Protocol, Traits>::do_tuple_read_nonblock(
    const dbtuple *tuple, ValueReader &value_reader)
{
  ...
  dbtuple::ReadStatus stat;
  {
    ...
    if(is_snapshot_txn) {
      ...
      read_set.emplace_back(tuple, start_t, nullptr);
      stat = tuple->read(snapshot_tid, start_t, value_reader, this->string_allocator());
    ...
    const dbtuple *tuple, ValueReader &value_reader, bool readonly, ic3_profile* prof)
{
  ...
  prof->data[ic3_profile::level::LEVEL2] += rdtsc() - beg_l2;
  uint64_t beg_l3 = rdtsc();
  // do the actual tuple read
  dbtuple::ReadStatus stat;
  {
    PERF_DECL(static std::string probe0_name(std::string(__PRETTY_FUNCTION__) + std::string(":do_read:")));
    ...
    if(is_snapshot_txn) {
      ...
      if(readonly) {
        ...
        // printf("do_tuple_read acquire write lock %lx\n", (uint64_t)t_ptr);
        // mcs_rwlock::qnode_t* node = new mcs_rwlock::qnode_t();
        // mcs_rwlock::qnode_t* node = read_for_write_set.back().get_lock_node();
        //WZG: write lock
        typename lock_set_map::iterator it_r = find_write_lock_set(t_ptr);
        ...
        prof->data[ic3_profile::level::LEVEL3] += rdtsc() - beg_l3;
      }

      uint64_t beg_l4 = rdtsc();
      stat = tuple->read(snapshot_tid, start_t, value_reader, this->string_allocator());
      prof->data[ic3_profile::level::LEVEL4] += rdtsc() - beg_l4;
