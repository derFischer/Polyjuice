    ...
    bool readonly)
{
  ...
  if (found) {
    ...
    return t.do_tuple_read(tuple, value_reader, readonly);
  } else {
...
retry:
  ...
  if (!insert) {
    ...
    typename Transaction<Traits>::lock_set_map::iterator it_r = t.find_write_lock_set(px);

    if(it_r == t.write_lock_set.end()) {      
      if(!px->start_write(t.get_tid(), t.not_sorted()))
      {
    ...
    const typename concurrent_btree::node_opaque_t *n, uint64_t version)
{
  ...
  // if (t->do_tuple_read_nonblock(tuple, *value_reader))
  if (t->do_tuple_read(tuple, *value_reader, true))
