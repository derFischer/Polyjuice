...
namespace TPCE
{
    //
    // Description:
    // A class for managing all data files. There are several important characteristics
    // of this class.
    //
    ...
    // environment.
    //
    // - Load Type. This class supports both immediate and lazy loading of the data files.
    // The type of load can be specified at object creation time.
    //
    ...
    // alter things are not considered const but lazy loading of a file on file access
    // is considered const even though that lazy load may trigger an exception (e.g. if
    ...
    private:
        ...
        // Directory where the flat_in files are located.
        std::string dir;
        ...
        // Scaling Data File Abstractions (mutable to support const-appearing lazy load)
    ...
    public:
        // Load files at object construction or defer for lazy load.
        enum LoadType {
            IMMEDIATE_LOAD,
            LAZY_LOAD
        };
