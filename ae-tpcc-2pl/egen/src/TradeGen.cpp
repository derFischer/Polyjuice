...
*
*/
CTradeGen::CTradeGen(const DataFileManager& dfm,
    ...
    , m_AddressTable(dfm, iCustomerCount, iStartFromCustomer, true, bCacheEnabled)  // only customer addresses
...
void CTradeGen::GenerateTradeCharge()
{
    ...
    for (i=0; i<m_ChargeFile.size(); ++i)
    {
        ...
        if ( GetCurrentCustTier() == chargeRow.CH_C_TIER() )
        {
            ...
            //if (!strcmp(tradeTypeRow.TT_ID_CSTR(), chargeRow.CH_TT_ID_CSTR()))
...
void CTradeGen::GenerateTradeCommission()
{
    ...
    for (UINT i = iStartIndex; i < iEndIndex; i++)
    {
        ...
        if ( iTradeQty >= commissionRow.CR_FROM_QTY() && iTradeQty <= commissionRow.CR_TO_QTY() )
        {
            ...
            m_CompletedTradeInfo.Commission = (iTradeQty * GetCurrentTradePrice()) *
                commissionRow.CR_RATE() / 100.0;
            return;
        }
    }
...
void CTradeGen::GenerateSettlementRow()
{
    ...
    m_TradeRow.m_Settlement.SE_CASH_DUE_DATE.Set(0,0,0,0);      // zero out time portion
