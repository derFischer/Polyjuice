...
class rdtsc_timer {
private:
  rdtsc_timer(const rdtsc_timer &) = delete;
  rdtsc_timer &operator=(const rdtsc_timer &) = delete;
  rdtsc_timer(rdtsc_timer &&) = delete;
...
public:
  ...
  lap()
  {
    ...
    uint64_t t1 = rdtsc();
    start = t1;
    return (uint64_t)((double)(t1 - t0) / CPU_FREQ_MHZ);
  }
  ...
  lap_ms()
  {
    ...
    uint64_t t1 = rdtsc();
    start = t1;
    return ((double)(t1 - t0) / (1000.0 * CPU_FREQ_MHZ));
  }
