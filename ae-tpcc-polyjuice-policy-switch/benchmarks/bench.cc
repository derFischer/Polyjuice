...
volatile unsigned int second_mark = 0;
int verbose = 0;
...
bench_runner::run()
{
  ...
  {
    size_t n_commits_sec = 0;
    size_t n_aborts_sec = 0;
    for (size_t i = 0; i < nthreads; i++) {
      n_commits_sec += workers[i]->ntxn_commits[sec];
      n_aborts_sec += workers[i]->ntxn_aborts[sec];
    }
    cout << "Second " << sec << " throughput " << n_commits_sec << ", abort " << n_aborts_sec << endl;;
  }
  // cout << "COMMIT_BREAKDOWN ";
