...
public:
  ...
  is_initial()
  {
    ...
    ALWAYS_ASSERT(false);
#else
    return (sub_version != MAX_TID && is_latest());
#endif
  }
  ...
  wait_initial()
  {
    ...
    ALWAYS_ASSERT(false);
#else
    while(sub_version == MAX_TID && is_latest())
    {
      __sync_synchronize ();
    }
    ...
    //fprintf(stderr, "[wait_initial] read tuple %lx \n",this);
    //ALWAYS_ASSERT(false);
      return false;
  #endif
  }
  ...
  append_entry(access_entry* e)
  {
    ...
    //fprintf(stderr, "tuple: [%lx] link entry %lx\n", this, e);
    if(NULL != access_tail) {

      ALWAYS_ASSERT(access_head != NULL);
      ...
      ALWAYS_ASSERT(access_head == NULL && access_tail == NULL);
  ...
  unlink_entry(access_entry *e)
  {
    ...
    if (access_tail == e && access_head == e)
    {
      ...
      INVARIANT(e->next != nullptr && e->prev != nullptr);
