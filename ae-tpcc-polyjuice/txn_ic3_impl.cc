    ...
    vector<unsigned> assignment)
{
    ...
    for (auto idx : assignment) {
      ...
      for (size_t k = idx; k < NMAXCORES; k += g_nworkers) {
        ...
        for (auto px : pxs) {
          ...
          if (transaction_ic3_static::EpochId(px->header()->last_tid_) >=
              cur_sync_epoch_ex + g_max_lag_epochs) {
  ...
  #define PXLEN(px) ((px)->curoff_)
          ...
          auto last_tid_cid = transaction_ic3_static::CoreId(px->header()->last_tid_);
          ...
          const uint64_t px_epoch =
            transaction_ic3_static::EpochId(px->header()->last_tid_);
          INVARIANT(
              transaction_ic3_static::CoreId(px->header()->last_tid_) ==
              px->core_id_);
...
sleep_ro_epoch()
{
  const uint64_t sleep_ns = transaction_ic3_static::ReadOnlyEpochUsec * 1000;
  struct timespec t;
  t.tv_sec  = sleep_ns / ONE_SECOND_NS;
  t.tv_nsec = sleep_ns % ONE_SECOND_NS;
  nanosleep(&t, nullptr);
...
void
transaction_ic3_static::PurgeThreadOutstandingGCTasks()
{
...
void
transaction_ic3_static::clean_up_to_including(threadctx &ctx, uint64_t ro_tick_geq)
{
  INVARIANT(!rcu::s_instance.in_rcu_region());
...
#define EXIT_RCU() \
  ...
  for (auto it = q.begin(); it != q.end(); ++it, ++n, ++niters_with_rcu) {
    ...
    if (!delent.key_.get_flags()) {
      ...
      ::mcs_lock_guard<dbtuple> lg_tuple(delent.tuple(), &node);
#else      
...
aligned_padded_elem<transaction_ic3_static::hackstruct>
  transaction_ic3_static::g_hack;
aligned_padded_elem<transaction_ic3_static::flags>
  transaction_ic3_static::g_flags;
percore_lazy<transaction_ic3_static::threadctx>
  transaction_ic3_static::g_threadctxs;
event_counter
  transaction_ic3_static::g_evt_worker_thread_wait_log_buffer(
      "worker_thread_wait_log_buffer");
event_counter
  transaction_ic3_static::g_evt_dbtuple_no_space_for_delkey(
      "dbtuple_no_space_for_delkey");
event_counter
  transaction_ic3_static::g_evt_proto_gc_delete_requeue(
      "proto_gc_delete_requeue");
event_avg_counter
  transaction_ic3_static::g_evt_avg_log_entry_size(
      "avg_log_entry_size");
event_avg_counter
  transaction_ic3_static::g_evt_avg_proto_gc_queue_len(
