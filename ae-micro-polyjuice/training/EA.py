...
class EA:
    ...
    def mutate(self, sample, decay_rate):
        ...
        # wait_info1 = wait_info1 - wait_info1 % 2
        ...
        # wait_info2 = wait_info2 - wait_info2 % 2
        ...
        # ait_info3 = wait_info3 - wait_info3 % 2

        # wait_info4 mutation
        parent_wait_info4 = sample.np_wait_info4
        wait_info4_mut = (np.random.rand(len(parent_wait_info4)) < self.mut_frac).astype(int)
        # step width
        wait_info4_step_width = int(decay_rate * wait_info_act_count[3])
        wait_info4_step = ((np.random.rand(len(parent_wait_info4)) * wait_info4_step_width) + 1).astype(int) * ((np.random.rand(len(parent_wait_info4)) < .5) * 2 - 1).astype(int)
        # do mutation
        wait_info4 = (parent_wait_info4 + wait_info4_mut * wait_info4_step) % (wait_info_act_count[3])
        # wait_info4 = wait_info4 - wait_info4 % 2

        # wait_info5 mutation
        parent_wait_info5 = sample.np_wait_info5
        wait_info5_mut = (np.random.rand(len(parent_wait_info5)) < self.mut_frac).astype(int)
        # step width
        wait_info5_step_width = int(decay_rate * wait_info_act_count[4])
        wait_info5_step = ((np.random.rand(len(parent_wait_info5)) * wait_info5_step_width) + 1).astype(int) * ((np.random.rand(len(parent_wait_info5)) < .5) * 2 - 1).astype(int)
        # do mutation
        wait_info5 = (parent_wait_info5 + wait_info5_mut * wait_info5_step) % (wait_info_act_count[4])
        # wait_info5 = wait_info5 - wait_info5 % 2

        # wait_info6 mutation
        parent_wait_info6 = sample.np_wait_info6
        wait_info6_mut = (np.random.rand(len(parent_wait_info6)) < self.mut_frac).astype(int)
        # step width
        wait_info6_step_width = int(decay_rate * wait_info_act_count[5])
        wait_info6_step = ((np.random.rand(len(parent_wait_info6)) * wait_info6_step_width) + 1).astype(int) * ((np.random.rand(len(parent_wait_info6)) < .5) * 2 - 1).astype(int)
        # do mutation
        wait_info6 = (parent_wait_info6 + wait_info6_mut * wait_info6_step) % (wait_info_act_count[5])
        # wait_info6 = wait_info6 - wait_info6 % 2

        # wait_info7 mutation
        parent_wait_info7 = sample.np_wait_info7
        wait_info7_mut = (np.random.rand(len(parent_wait_info7)) < self.mut_frac).astype(int)
        # step width
        wait_info7_step_width = int(decay_rate * wait_info_act_count[6])
        wait_info7_step = ((np.random.rand(len(parent_wait_info7)) * wait_info7_step_width) + 1).astype(int) * ((np.random.rand(len(parent_wait_info7)) < .5) * 2 - 1).astype(int)
        # do mutation
        wait_info7 = (parent_wait_info7 + wait_info7_mut * wait_info7_step) % (wait_info_act_count[6])
        # wait_info7 = wait_info7 - wait_info7 % 2

        # wait_info8 mutation
        parent_wait_info8 = sample.np_wait_info8
        wait_info8_mut = (np.random.rand(len(parent_wait_info8)) < self.mut_frac).astype(int)
        # step width
        wait_info8_step_width = int(decay_rate * wait_info_act_count[7])
        wait_info8_step = ((np.random.rand(len(parent_wait_info8)) * wait_info8_step_width) + 1).astype(int) * ((np.random.rand(len(parent_wait_info8)) < .5) * 2 - 1).astype(int)
        # do mutation
        wait_info8 = (parent_wait_info8 + wait_info8_mut * wait_info8_step) % (wait_info_act_count[7])
        # wait_info8 = wait_info8 - wait_info8 % 2

        # wait_info9 mutation
        parent_wait_info9 = sample.np_wait_info9
        wait_info9_mut = (np.random.rand(len(parent_wait_info9)) < self.mut_frac).astype(int)
        # step width
        wait_info9_step_width = int(decay_rate * wait_info_act_count[8])
        wait_info9_step = ((np.random.rand(len(parent_wait_info9)) * wait_info9_step_width) + 1).astype(int) * ((np.random.rand(len(parent_wait_info9)) < .5) * 2 - 1).astype(int)
        # do mutation
        wait_info9 = (parent_wait_info9 + wait_info9_mut * wait_info9_step) % (wait_info_act_count[8])
        # wait_info9 = wait_info9 - wait_info9 % 2

        # wait_info10 mutation
        parent_wait_info10 = sample.np_wait_info10
        wait_info10_mut = (np.random.rand(len(parent_wait_info10)) < self.mut_frac).astype(int)
        # step width
        wait_info10_step_width = int(decay_rate * wait_info_act_count[9])
        wait_info10_step = ((np.random.rand(len(parent_wait_info10)) * wait_info10_step_width) + 1).astype(int) * ((np.random.rand(len(parent_wait_info10)) < .5) * 2 - 1).astype(int)
        # do mutation
        wait_info10 = (parent_wait_info10 + wait_info10_mut * wait_info10_step) % (wait_info_act_count[9])
        # wait_info10 = wait_info10 - wait_info10 % 2
        ...
        for i in range(WAIT_SPACE):
            if wait[i] == 0: # this is a no_wait action
                ...
                wait_info4[i] = parent_wait_info4[i]
                wait_info5[i] = parent_wait_info5[i]
                wait_info6[i] = parent_wait_info6[i]
                wait_info7[i] = parent_wait_info7[i]
                wait_info8[i] = parent_wait_info8[i]
                wait_info9[i] = parent_wait_info9[i]
                wait_info10[i] = parent_wait_info10[i]

        mutate_res =  Sample(access, wait, piece, 
    ...
    def store_actions(self, access, wait, piece, 
                        wait_info1, wait_info2, wait_info3, 
                        wait_info4, wait_info5, wait_info6, 
                        wait_info7, wait_info8, wait_info9, 
                        wait_info10):
        ...
        self.wait_info_act4.extend(wait_info4)
        self.wait_info_act5.extend(wait_info5)
        self.wait_info_act6.extend(wait_info6)
        self.wait_info_act7.extend(wait_info7)
        self.wait_info_act8.extend(wait_info8)
        self.wait_info_act9.extend(wait_info9)
        self.wait_info_act10.extend(wait_info10)
    ...
    def Evaluate(self, round_id, generations, command, load_per_sample):
        ...
        for _, child in enumerate(sample_buffer):
            self.store_actions(child.access, child.wait, child.piece, \
                               ...
                               child.wait_info4, child.wait_info5, child.wait_info6, \
                               child.wait_info7, child.wait_info8, child.wait_info9, \
                               child.wait_info10)
