...
class EA:
    ...
    def mutate(self, sample, decay_rate):
        ...
        for i in range(ACCESSES):
            for j in range(count_per_access):
                # hack !!!
                if piece[i * count_per_access + 2] == 0:
                    wait[i * count_per_access + 2] = sample.np_wait[i * count_per_access + 2]
                    wait_info1[i * count_per_access + 2] = parent_wait_info1[i * count_per_access + 2]
                    wait_info2[i * count_per_access + 2] = parent_wait_info2[i * count_per_access + 2]
                    wait_info3[i * count_per_access + 2] = parent_wait_info3[i * count_per_access + 2]
                if piece[i * count_per_access + 3] == 0:
                    wait[i * count_per_access + 3] = sample.np_wait[i * count_per_access + 3]
                    wait_info1[i * count_per_access + 3] = parent_wait_info1[i * count_per_access + 3]
                    wait_info2[i * count_per_access + 3] = parent_wait_info2[i * count_per_access + 3]
                    wait_info3[i * count_per_access + 3] = parent_wait_info3[i * count_per_access + 3]

        mutate_res =  Sample(access, wait, piece, wait_info1, wait_info2, wait_info3, txn_buf_size, backoff)
        
        if self.training_stage < 1:     # wait_info align for different op_types for stage == 0
            mutate_res.align_wait_info()
        elif self.training_stage < 2:     # contention mutation only for stage >= 2
            mutate_res.align_contention_level()
