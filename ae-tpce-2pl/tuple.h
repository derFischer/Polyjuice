...
public:
  ...
  inline bool start_read(uint64_t txn_id, bool not_sorted = true)
  {
    // fprintf(stderr, "start read  %lx %lx\n", (uint64_t)this, (uint64_t)txn_id);
    // FIX!!!
    return rwlock.lockR(txn_id, not_sorted);
  }
  ...
  inline bool start_write(uint64_t txn_id, bool not_sorted = true)
  {
    // fprintf(stderr, "start write  %lx %lx\n", (uint64_t)this, (uint64_t)txn_id);
    return rwlock.lockW(txn_id, not_sorted);
    // lock(true);
  }
