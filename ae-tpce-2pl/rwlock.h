...
public:
    ...
    {
        return lock_get(LOCK_WR, tid, not_sorted);
    }
    ...
    {
        return lock_get(LOCK_RD, tid, not_sorted);
    }
...
private:
	...
	bool conflict_lock(uint16_t l1, uint16_t l2)
    {
        if (l1 == LOCK_NONE || l2 == LOCK_NONE)
            return false;
        else if (l1 == LOCK_WR || l2 == LOCK_WR)
            return true;
        else
            return false;
    }
	LockEntry * get_entry() {
        LockEntry *tmp = new LockEntry();
        tmp->type = LOCK_NONE;
        tmp->tid = 0;
        tmp->lock_ready = false;
        tmp->next = NULL;
        tmp->prev = NULL;
