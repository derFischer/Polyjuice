...
//
// Include our header first to make sure it is self-contained
//
#include "../inc/TestUtilities.h"
...
namespace EGenTestCommon
{
    ...
    void AddTestCase( const char *const testCaseName, void (pTestCaseMethod )(const std::deque<std::string>&), const std::deque<std::string>& fields, boost::unit_test::test_suite* testSuite )
    {
        ...
        testCase = BOOST_TEST_CASE( boost::bind( pTestCaseMethod, fields ));
    ...
    {
        return( abs( amount1 - amount2 ) < threshold );
    }
    ...
    {
        return( CloseEnough( amount1, amount2, 0.001 ));
    }
