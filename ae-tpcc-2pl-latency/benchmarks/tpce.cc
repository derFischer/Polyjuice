...
public:
    util::rdtsc_timer market_feed_timer_;
...
//#endif
    auto ek = Encode(k);
    auto kv = Encode(obj_buf, v);
    void *txn = tpce_loader_->get_txn();
    tbl_NEWS_ITEM(1)->insert(txn, ek, kv);
    tpce_loader_->inserted_row();
}
...
void tpce_worker::remove_holding(
	...
	tbl_HOLDING_H_CA_ID(1/*TODO*/)->remove(txn, Encode(obj_key0, k));
    // delete
    tbl_HOLDING(1/*TODO*/)->remove(txn, Encode(obj_key0, kv.key));
}
...
void tpce_worker::remove_trade_request(
    ...
    tbl_TRADE_REQUEST(1/*TODO*/)->remove(txn,Encode(obj_key0, k));
    // delete secondary index TRADE_REQUEST_TR_B_ID
    tbl_TRADE_REQUEST_TR_B_ID(1/*TODO*/)->remove(txn,Encode(obj_key0, k1));
    // delete secondary index TRADE_REQUEST_TR_S_SYMB
    tbl_TRADE_REQUEST_TR_S_SYMB(1/*TODO*/)->remove(txn,Encode(obj_key0, k2));
}
...
tpce_worker::txn_broker_volume()
{
	...
	void *txn = db->new_txn(
            txn_flags | read_only_mask, arena, txn_buf(), abstract_db::HINT_DEFAULT);
...
tpce_worker::txn_market_feed()
{
    ...
    try {
        ...
        for (mfi = 0; mfi < marketfeedIndex; mfi++) {
            ...
            tbl_LAST_TRADE(1/*TODO*/)->put(txn, Encode(str(), k), Encode(obj_v, v_lt_new));
        }
        ...
        for (mfi = 0; mfi < marketfeedIndex; mfi++) {
            ...
            for (auto &it : request_list) {
                ...
                if (unlikely(!(tbl_TRADE_REQUEST(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v)))) {
#ifdef EARLY_ABORT
                    db->abort_txn(txn);
        ...
        for (auto &it : request_list) {
            ...
            tbl_TRADE(1/*TODO*/)->put(txn, Encode(str(), k), Encode(obj_v, v_t_new));
        }
        ...
        // i [TRADE_HISTORY]
        {
            ...
            for (auto &it : request_list) {
                ...
                v.TH_DTS = now_dts;
                tbl_TRADE_HISTORY(1/*TODO*/)->insert(txn, Encode(str(), k), Encode(obj_v, v));
                ret += Size(v);
            }
        }
...
tpce_worker::txn_trade_order()
{
    ...
    inline_str_fixed_tpce<TPCE::sm_cTAX_ID_len> tax_id;
    {
        ...
        CUSTOMER::key k;
        k.C_ID = cust_id;
		ALWAYS_ASSERT(tbl_CUSTOMER(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v));
    ...
    // Raw Piece: P4 -- r [ACCOUNT_PERMISSION](AP_F_NAME, AP_L_NAME, AP_ACL)
    {
        ...
        if (strcmp(cust_l_name.data(), TxnReq.exec_l_name) != 0 ||
            ...
            if (unlikely(!tbl_ACCOUNT_PERMISSION(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v))) {
		        db->abort_txn(txn);
    		    return txn_result(true, 0);
    	  	} else {
    ...
    // Raw Piece: P5 -- scan [SECURITY, COMPANY]
    {
        ...
        if (TxnReq.symbol[0] == '\0') {
            ...
            // scan [SECURITY_S_CO] r [SECURITY]
            {
				...
				ALWAYS_ASSERT(tbl_SECURITY(1/*TODO*/)->get(txn, Encode(obj_key0, sk), obj_v));
            ...
            {
                const SECURITY::key k(symbol);
				ALWAYS_ASSERT(tbl_SECURITY(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v));
            ...
            // r [COMPANY]
            {
				...
				ALWAYS_ASSERT(tbl_COMPANY(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v));
    ...
    // Raw Piece: P6 -- r [TRADE_TYPE]
    {
		...
		ALWAYS_ASSERT(tbl_TRADE_TYPE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v));
    ...
    // Conflict Piece: P7 -- r [HOLDING_SUMMARY]
    {
        ...
        if (TxnReq.type_is_margin) {
            ...
            if (!hs_found) {
                auto it_find = hs_list.begin();
                hs_list.insert(it_find, std::make_pair(0, symbol));
            }
        } else {
            const HOLDING_SUMMARY::key k(TxnReq.acct_id, symbol);
			bool has_hs = tbl_HOLDING_SUMMARY(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v);
			if(has_hs){
            	HOLDING_SUMMARY::value v_hs_temp;
        ...
        if (type_is_sell) { // sell order
            if (hs_qty > 0) {
                ...
                for (auto h_t_id : tid_set) {
                    ...
                    if (unlikely(!tbl_HOLDING(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v))) {
#ifdef EARLY_ABORT
                        db->abort_txn(txn);
                        return txn_result(false, 0);
#else
    ...
    float requested_price = 0.0;
    {
        ...
        if (TxnReq.type_is_margin) {
            ...
            for (auto &it : hs_list) {
				...
				ALWAYS_ASSERT(tbl_LAST_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v));
                LAST_TRADE::value v_lt_temp;
    ...
    // Raw Piece: P10 -- r [CUSTOMER_TAXRATE, TAX_RATE]
    {
        ...
        if (sell_value > buy_value && (tax_status == 1 || tax_status == 2)) {
            ...
            for (size_t si = 0; si < cb.size(); si++) {
				...
				ALWAYS_ASSERT(tbl_TAX_RATE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v));
    ...
    // Raw Piece: P12 -- r [CHARGE]
    {
		...
		ALWAYS_ASSERT(tbl_CHARGE(1/*TODO*/)->get(txn, Encode(obj_key0, k), obj_v));
        ...
        if (TxnReq.frame_to_execute == 1) {
            ...
            for (int i = 0; i < TxnReq.max_trades; i++) {
                if (num_updated < TxnReq.max_updates) {
                    ...
                    num_updated++;
                    tbl_TRADE(1/*TODO*/)->put(txn, Encode(str(), k_t), Encode(obj_v, v_t_new));
                } else {
                    const TRADE::key k_t(TxnReq.trade_id[i]);
                    ALWAYS_ASSERT(tbl_TRADE(1/*TODO*/)->get(txn, Encode(obj_key0, k_t), obj_v));
                    TRADE::value v_t_temp;
