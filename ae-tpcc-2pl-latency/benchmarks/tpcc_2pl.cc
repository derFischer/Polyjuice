...
class tpcc_2pl_worker_mixin : private _dummy {

#define DEFN_TBL_INIT_X(name) \
  , tbl_ ## name ## _vec(partitions.at(#name))

public:
  tpcc_2pl_worker_mixin(const map<string, vector<abstract_ordered_index *>> &partitions) :
    _dummy() // so hacky...
...
string tpcc_2pl_worker_mixin::NameTokens[] =
  {
    string("BAR"),
    string("OUGHT"),
    string("ABLE"),
...
class tpcc_2pl_worker : public bench_worker, public tpcc_2pl_worker_mixin {
public:
  // resp for [warehouse_id_start, warehouse_id_end)
  tpcc_2pl_worker(unsigned int worker_id,
    ...
    : bench_worker(worker_id, true, seed, db,
      ...
      tpcc_2pl_worker_mixin(partitions),
  ...
  TxnNewOrder(bench_worker *w)
  {
    ...
    return static_cast<tpcc_2pl_worker *>(w)->txn_new_order();
  }
  ...
  TxnDelivery(bench_worker *w)
  {
    ...
    return static_cast<tpcc_2pl_worker *>(w)->txn_delivery();
  }
  ...
  TxnPayment(bench_worker *w)
  {
    ...
    return static_cast<tpcc_2pl_worker *>(w)->txn_payment();
  }
  ...
  TxnOrderStatus(bench_worker *w)
  {
    ...
    return static_cast<tpcc_2pl_worker *>(w)->txn_order_status();
  }
  ...
  TxnStockLevel(bench_worker *w)
  {
    ...
    return static_cast<tpcc_2pl_worker *>(w)->txn_stock_level();
  }
...
class tpcc_warehouse_loader : public bench_loader, public tpcc_2pl_worker_mixin {
public:
...
class tpcc_item_loader : public bench_loader, public tpcc_2pl_worker_mixin {
public:
...
class tpcc_stock_loader : public bench_loader, public tpcc_2pl_worker_mixin {
public:
...
class tpcc_district_loader : public bench_loader, public tpcc_2pl_worker_mixin {
public:
...
class tpcc_customer_loader : public bench_loader, public tpcc_2pl_worker_mixin {
public:
...
class tpcc_order_loader : public bench_loader, public tpcc_2pl_worker_mixin {
public:
...
tpcc_2pl_worker::txn_result
tpcc_2pl_worker::txn_new_order()
{
...
tpcc_2pl_worker::txn_result
tpcc_2pl_worker::txn_order_status()
{
  const uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
  const uint districtID = RandomNumber(r, 1, NumDistrictsPerWarehouse());
...
tpcc_2pl_worker::txn_result
tpcc_2pl_worker::txn_stock_level()
{
  const uint warehouse_id = PickWarehouseId(r, warehouse_id_start, warehouse_id_end);
...
void
tpcc_2pl_do_test(abstract_db *db, int argc, char **argv)
{
  // parse options
  optind = 1;
