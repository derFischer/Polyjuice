...
public:
      ...
      warehouse_id_end(warehouse_id_end)
  {
    ...
    // NDB_MEMSET(&dist_last_id[0], 0, sizeof(dist_last_id));
    if (verbose) {
...
private:
  ...
  // int32_t dist_last_id[10]; // XXX(stephentu): hack
...
tpcc_worker::txn_new_order()
{
  ...
  try {
    ...
    for (uint ol_number = 1; ol_number <= numItems; ol_number++) {
      ...
      itemPrices.push_back(v_i->i_price);
    }
...
tpcc_worker::txn_delivery()
{
  ...
  try {
    ...
    for (uint d = 1; d <= NumDistrictsPerWarehouse(); d++) {
      ...
      tbl_new_order(warehouse_id)->remove(txn, Encode(str(), new_order_keys[d - 1]));
    }
