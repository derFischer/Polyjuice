...
class MultiBaseline:
    def __init__(self, baseline_number):
        ...
        self.reward_signal_wait_info1, self.reward_signal_wait_info2, self.reward_signal_wait_info3, self.reward_signal_wait_info4 = [], [], [], []
    ...
    def store_reward_signal(self, result):
        ...
        self.reward_signal_wait_info4.extend(result[6])

    def samples_different_action(self, access, wait, piece, waitinfo1, waitinfo2, waitinfo3, waitinfo4):
        # get a all True form
        ...
        for j in range(self.baseline_number):
            diff = self.baselines[j].different_action(\
                                access, wait, piece, waitinfo1, waitinfo2, waitinfo3, waitinfo4)
    ...
    def calculate_reward(self, reward):
        ...
        waitinfo1_rs, waitinfo2_rs, waitinfo3_rs, waitinfo4_rs = \
        [0] * (len(reward) * WAIT_SPACE), [0] * (len(reward) * WAIT_SPACE), [0] * (len(reward) * WAIT_SPACE), [0] * (len(reward) * WAIT_SPACE)

        for i in range(self.baseline_number):

            # calculate discount_reward for each slot
            access_dr, wait_dr, piece_dr = [], [], []
            waitinfo1_dr, waitinfo2_dr, waitinfo3_dr, waitinfo4_dr = [], [], [], []
            ...
            waitinfo4_dr = np.array(waitinfo4_dr) - self.baselines[i].reward
            ...
            waitinfo4_rs = waitinfo4_rs + ratio[i] * waitinfo4_dr * ((waitinfo4_dr > 0) | self.reward_signal_wait_info4)

        return access_rs, wait_rs, piece_rs, waitinfo1_rs, waitinfo2_rs, waitinfo3_rs, waitinfo4_rs
...
class Baseline:
    def __init__(self, access = [], wait = [], piece = [], \
                 waitinfo1 = [], waitinfo2 = [], waitinfo3 = [], waitinfo4 = [], \
                 reward = 0):
        if access == []:
            self.set = False
        else:
        ...
        self.sample = Sample(access, wait, piece, waitinfo1, waitinfo2, waitinfo3, waitinfo4)
        self.reward = reward

    def setSample(self, access, wait, piece, waitinfo1, waitinfo2, waitinfo3, waitinfo4, reward):
        self.set = True
        self.sample.set_sample(access, wait, piece, waitinfo1, waitinfo2, waitinfo3, waitinfo4)
        self.reward = reward
    ...
    def SetSampleWithAnotherBaseline(self, baseline):
        self.setSample(baseline.sample.access, baseline.sample.wait, baseline.sample.piece, \
                       baseline.sample.wait_info1, baseline.sample.wait_info2, baseline.sample.wait_info3, baseline.sample.wait_info4, \
                       baseline.reward)

    def __lt__(self, r):
        return self.reward < r.reward

    def different_action(self, access, wait, piece, waitinfo1, waitinfo2, waitinfo3, waitinfo4):
        if self.set == False:
...
class PolicyGradient:
    ...
    def __init__(self, log_dir, kid_dir, learning_rate,rd,output_graph=False):
        ...
        self.wait_info1_p, self.wait_info2_p, self.wait_info3_p, self.wait_info4_p = [], [], [], []

        self.ep_access_rs, self.ep_wait_rs, self.ep_piece_rs = [], [], []
        self.ep_waitinfo1_rs, self.ep_waitinfo2_rs, self.ep_waitinfo3_rs, self.ep_waitinfo4_rs = [], [], [], []

        self.ep_access_act, self.ep_wait_act, self.ep_piece_act = [], [], []
        self.ep_wait_info_act1, self.ep_wait_info_act2, self.ep_wait_info_act3, self.ep_wait_info_act4 = [], [], [], []
    ...
    def _build_net(self):
        with tf.name_scope('inputs'):
            ...
            self.tf_wait_info_vt4 = tf.placeholder(tf.float32, [None, ], name="wait_info_value4")
            ...
            self.tf_wait_info_act4 = tf.placeholder(tf.int32, [None, ], name="wait_info_act4")
        ...
        self.wait_info_action4 = tf.nn.softmax(tf.Variable(tf.random_normal(shape=[WAIT_SPACE, wait_info_act_count[3]], mean=0, stddev=1), name='wait_info_action4'), axis = 1)
        ...
        with tf.name_scope('reward'):
            ...
            self.wait_info_action4 = tf.add(self.wait_info_action4, 0.000001)
            ...
            wait_info_act4 = tf.reshape((tf.one_hot(self.tf_wait_info_act4, wait_info_act_count[3])), [-1, WAIT_SPACE * wait_info_act_count[3]])
            wait_info_act4_prob = tf.reshape((wait_info_act4 * (tf.reshape(self.wait_info_action4, [WAIT_SPACE * wait_info_act_count[3]]))), [-1, wait_info_act_count[3]])
            wait_info_act4_prob = -tf.log(tf.reduce_sum(wait_info_act4_prob, axis = 1))
            ...
            self.reward = tf.divide(tf.reduce_sum(access_act_prob * self.tf_access_vt) + \
                                    ...
                                    tf.reduce_sum(wait_info_act3_prob * self.tf_wait_info_vt3) + \
                                    tf.reduce_sum(wait_info_act4_prob * self.tf_wait_info_vt4), self.tf_samples_count)

        with tf.name_scope('train'):
            self.train_op = tf.train.GradientDescentOptimizer(learning_rate = self.learning_rate).minimize(self.reward)

    def update_policy(self):
        access_p, wait_p, piece_p, wait_info1_p, wait_info2_p, wait_info3_p, wait_info4_p = \
        self.sess.run([self.access_action, self.wait_action, self.piece_action, \
                       self.wait_info_action1, self.wait_info_action2, self.wait_info_action3, self.wait_info_action4])

        self.policy.set_prob(access_p, wait_p, piece_p, \
                             wait_info1_p, wait_info2_p, wait_info3_p, wait_info4_p)
    ...
    def record_reward(self, round_id, reward, previous_samples, idx):
        ...
        waitinfo4 = self.ep_wait_info_act4[previous_samples * WAIT_SPACE : (previous_samples + 1) * WAIT_SPACE]

        if reward > self.baselines.baselines[0].reward:
            baseline_ = Baseline(access, wait, piece, waitinfo1, waitinfo2, waitinfo3, waitinfo4, reward)
    ...
    def set_baseline(self, access, wait, piece, \
        ...
        for i in range(samples):
            ...
            if r > self.baselines.baselines[0].reward:
                ...
                waitinfo4_t = wait_info4[i * WAIT_SPACE : (i + 1) * WAIT_SPACE]
                baseline_ = Baseline(access_t, wait_t, piece_t, \
                                     waitinfo1_t, waitinfo2_t, waitinfo3_t, waitinfo4_t, \
                                     r)
                self.baselines.insert_baseline(baseline_)
                self.high_lr_lease = HIGH_LR_LEASE
    ...
    def learn(self, idx, lr, generations):
        ...
        self.sess.run(self.train_op, feed_dict={
             ...
             self.tf_wait_info_act4: self.ep_wait_info_act4,
             ...
             self.tf_wait_info_vt4: self.ep_waitinfo4_rs,
    ...
    def rollback(self, index, fail_to_exe):
        ...
        self.ep_wait_info_act4 = self.ep_wait_info_act4[:(index - fail_to_exe) * WAIT_SPACE] + self.ep_wait_info_act4[(index + 1 - fail_to_exe) * WAIT_SPACE :]