...
class PolicyAction {
    ...
    public:
    PolicyAction(bool access_a, AgentDecision wait_a, int access_wait_i[TXN_TYPE], bool piece_end_d)
    {
        ...
        {
            access_wait_info[i] = *(access_wait_i + i);
        }
    }
    PolicyAction(){}
    ~PolicyAction()
...
class PolicyGradient {
    ...
    PolicyAction* txn_commit_policy[TXN_TYPE][CONTENTION_LEVEL];
    ...
    ~PolicyGradient()
    {
        ...
        for(int i = 0; i < TXN_TYPE; ++i)
        {
            for(int j = 0; j < CONTENTION_LEVEL; ++j)
            {
                    delete txn_commit_policy[i][j];
            }
        }
        if (this)
        {
    ...
    PolicyAction* transfer_action(std::string action)
    {
        ...
        for (int i = 0; i < TXN_TYPE; ++i)
        {
            access_wait_info[i] = buffer[3 + i];
        }
        return new PolicyAction(access_action, wait_action, access_wait_info, piece_action);
    }

    uint16_t cal_contention_level(uint16_t contention)
    {
        return 0;
    }
